...
hallucinating intermediate steps that aren't on the page. It is also an
injection defense: the Scorer never receives free-form student text, only
the enumerated claims, so instructions hidden in an answer cannot reach it.

LLM calls go through `backend.grader.llm.get_llm(role)`. Each node passes
its role so routing/temperature come from settings:
  - Extractor → "vision" (Gemini by default)
  - Scorer, Justifier → "text" (Groq by default; model A)
  - Critic → "critic" (Groq by default; model B, cross-model)

The node code does not hardcode any provider or model.
"""
...
def _invoke(system: str, user_content: list[dict] | str, role: str = "text") -> str:
    """Call the role's LLM with a text-only or multimodal user message."""
    wait_turn()
    llm = get_llm(role)
...
def extractor_node(state: GradingState) -> dict:
    ...
    raw = _invoke(prompts.EXTRACTOR_SYSTEM, content, role="vision")
...
def scorer_node(state: GradingState) -> dict:
    ...
    raw = _invoke(prompts.SCORER_SYSTEM, user, role="text")
    parsed = parse_json(raw) or {}
...
def justifier_node(state: GradingState) -> dict:
    ...
    raw = _invoke(prompts.JUSTIFIER_SYSTEM, user, role="text")
    parsed = parse_json(raw) or {}
    return {
...
# Runs on the "critic" role so it can be a different model than the Scorer
# (cross-model check → removes self-preference bias).
# ===========================================================================
def critic_node(state: GradingState) -> dict:
    ...
    raw = _invoke(prompts.CRITIC_SYSTEM, user, role="critic")
    parsed = parse_json(raw) or {}
...
def critic_router(state: GradingState) -> str:
    """Return the name of the next node to run, or END to finish."""
